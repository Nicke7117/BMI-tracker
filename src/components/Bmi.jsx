export const Bmi = ({ bmiData }) => {
  return (
    <div>
    <div className="border-2 border-blue-400 h-5/6 m-auto col-start-1 row-start-4 rounded-xl w-5/6 relative mt-2">
      <img src="data:image/svg+xml;base64," className="w-1/12 absolute right-1 top-1" />
      <p className="text-center text-xl mt-1">3.12.2004</p>
      <p className="text-center text-base">Bmi: 23 - normal</p>
      <p className="text-center">Weight: 55 kg</p>
      <p className="text-center">Height: 177 cm</p>
    </div>

    </div>
  );
};
